  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(SCRIPT_URL, {
        ...
        body: JSON.stringify({ type: "awareness", answers, totalScore, timestamp: new Date().toISOString() }),
      });
    } catch (_) {}
    setSubmitting(false);
    setSubmitted(true);
          ...
          {!submitted ? (
            <motion.div key="form" exit={{ opacity: 0, y: -20 }} className="grid md:grid-cols-2 items-start" style={{ gap: 56 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={answered < AWARENESS_ITEMS.length || submitting}
                  style={{ marginTop: 8, padding: "14px 32px", borderRadius: 999, cursor: (answered < AWARENESS_ITEMS.length || submitting) ? "not-allowed" : "pointer", ...glassBase,
                    border: "1px solid rgba(253,230,138,0.35)", color: "#FDE68A", fontSize: 13, letterSpacing: "0.2em", opacity: (answered < AWARENESS_ITEMS.length || submitting) ? 0.45 : 1,
                    background: "rgba(253,230,138,0.07)", transition: "opacity 0.3s" }}>
                  {submitting ? "傳送中…" : `提交並查看分析 (${answered}/${AWARENESS_ITEMS.length})`}
                </motion.button>
              </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(SCRIPT_URL, {
        ...
        mode: "no-cors",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "survey", ...form, timestamp: new Date().toISOString() }),
      });
    } catch (_) {}
    setLoading(false);
    setSubmitted(true);
